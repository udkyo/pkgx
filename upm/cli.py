...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  upm install git vim          # Install packages
  upm remove old-package       # Remove packages
  upm update                   # Update package lists
  upm upgrade                  # Upgrade all packages
  upm upgrade git vim          # Upgrade specific packages
  upm search firefox           # Search for packages
  upm list-managers            # List available package managers
        """
    )
    ...
    if hasattr(args, 'manager') and args.manager:
        ...
        if not pm:
            ...
            print("Use 'upm list-managers' to see available managers")
            return 1
    else:
